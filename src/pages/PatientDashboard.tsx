...
export default function PatientDashboard() {
  ...
  if (!isAuthenticated) return (
    <div className="min-h-screen bg-[var(--bg-main)] flex flex-col relative overflow-hidden">
      ...
      <nav className="w-full h-20 flex items-center justify-between px-6 lg:px-12 z-10 border-b border-[var(--border-main)]">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
         <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md relative z-10">
           <div className="text-center mb-8">
             <div className="w-16 h-16 bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl mx-auto flex items-center justify-center mb-6">
               <ShieldCheck className="w-8 h-8 text-blue-400" />
             </div>
             <h1 className="text-3xl font-bold text-[var(--text-main)] tracking-tight">Patient Portal</h1>
             <p className="text-[var(--text-dim)] mt-2">Enter your email to view your records</p>
           </div>
             ...
             <div className="space-y-2">
               ...
               <div className="relative">
                 ...
                 <input 
                   ...
                   className="w-full bg-[var(--bg-main)]/50 border border-[var(--border-main)] rounded-xl pl-12 pr-4 py-4 text-[var(--text-main)] placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-blue-500/50 outline-none transition-all font-medium"
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] flex flex-col relative overflow-hidden text-[var(--text-main)]">
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-900/10 rounded-full blur-[120px] pointer-events-none" />
      
      <nav className="w-full h-20 flex items-center justify-between px-6 lg:px-12 z-10 border-b border-[var(--border-main)] bg-[var(--bg-main)]/50 backdrop-blur-md sticky top-0">
         <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/')}>
           <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-zinc-950 font-black text-xl">D</div>
           <span className="text-xl font-bold tracking-tight text-[var(--text-main)] hidden sm:block">DocBook.</span>
         </div>
         <button onClick={() => setIsAuthenticated(false)} className="text-sm text-zinc-400 hover:text-[var(--text-main)] transition-colors">Sign Out</button>
      </nav>
         ...
         <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {historyData.length === 0 ? (
               <div className="glass-panel p-12 rounded-3xl text-center border-dashed border-white/20">
                  ...
                  <h2 className="text-xl font-bold text-[var(--text-main)] mb-2">No records found</h2>
                  <p className="text-[var(--text-dim)] mb-6">You don't have any appointments linked to {email}.</p>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 <AnimatePresence>
                  {historyData.map(visit => (
                     <motion.div key={visit.id} layout className="glass-panel p-6 rounded-3xl flex flex-col justify-between border border-[var(--border-main)] relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                           ...
                           <h3 className="text-xl font-bold mb-1 text-[var(--text-main)]">{visit.specialty}</h3>
                        ...
                        {visit.status === 'confirmed' && (
                           <div className="mt-6 pt-4 border-t border-[var(--border-main)] flex gap-2 relative z-10">
                              <button className="flex-1 py-2 text-xs font-bold bg-white/5 hover:bg-white/10 text-[var(--text-main)] rounded-lg transition-colors border border-[var(--border-main)]">Details</button>
                              <button 
                                onClick={() => {
                        ...
                        {visit.status === 'completed' && (
                           <div className="mt-6 pt-4 border-t border-[var(--border-main)] relative z-10">
                               <button onClick={() => alert("Securely downloading encrypted Med Cert and Lab documents...")} className="w-full py-2 text-xs font-bold bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded-lg transition-colors border border-emerald-500/20 flex justify-center items-center gap-2">
