  ...
  return (
    <div className="space-y-6">
       <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
          <div>
             <h1 className="text-3xl font-bold tracking-tight text-[var(--text-main)] mb-1">Weekly Master Schedule</h1>
             <p className="text-zinc-400">7-day continuous clinic load overview.</p>
          </div>
          <div className="flex items-center gap-4 bg-[var(--bg-card)] border border-[var(--border-main)] rounded-xl p-1">
             ...
             <span className="text-sm font-bold text-[var(--text-main)] px-2">
       ...
       <div className="glass-panel rounded-2xl overflow-hidden overflow-x-auto border border-[var(--border-main)]">
          <div className="min-w-[800px]">
             {/* Header Row */}
             <div className="grid grid-cols-8 border-b border-[var(--border-main)] bg-[var(--bg-main)]/80">
                <div className="p-4 flex items-center justify-center text-xs font-bold text-[var(--text-dim)] uppercase tracking-widest border-r border-[var(--border-main)]">
                   Time
                </div>
                {weekDays.map(day => (
                   <div key={day.toISOString()} className={`p-4 flex flex-col items-center justify-center border-r border-[var(--border-main)] last:border-0 ${isSameDay(day, new Date()) ? 'bg-emerald-500/10 text-emerald-400' : 'text-zinc-300'}`}>
             ...
             <div className="divide-y divide-white/5">
                {allTimes.map(time => (
                   <div key={time} className="grid grid-cols-8 group hover:bg-white/[0.02] transition-colors">
                      <div className="p-4 flex items-center justify-center text-xs font-bold text-zinc-400 border-r border-[var(--border-main)] bg-[var(--bg-main)]/40">
                         {time}
                      </div>
                      ...
                      {weekDays.map(day => {
                         ...
                         return (
                            <div key={day.toISOString() + time} className={`p-2 border-r border-[var(--border-main)] last:border-0 relative h-24 ${isWeekendSlot ? 'bg-[var(--bg-card)]/50' : ''}`}>
                               {isWeekendSlot && (
                               ...
                               {!isWeekendSlot && booking && (
                                  <div className={`h-full w-full rounded-xl p-2 flex flex-col justify-between border ${
                                     ...
                                     <div className="flex items-center gap-1.5 mb-1 bg-[var(--bg-main)]/40 w-max px-1.5 py-0.5 rounded text-[10px] font-bold tracking-widest uppercase">
